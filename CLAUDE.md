...
- `firmware/include/User_Setup.h` — TFT_eSPI display pin/driver config, injected via `-include` in `platformio.ini` rather than editing the library's bundled copy (which `pio lib update` would overwrite).
- `firmware/src/sensors/` — one file pair per sensor (PMS5003, SCD30, SGP30, MiCS-4514, BH1750, MAX9814-based noise), each reading into the shared `SensorReadings` struct (`include/sensor_data.h`) via `sensors.h`'s `sensorsRead()`. Sensor code has no knowledge of display or network.
