...
void displayShowReadings(const SensorReadings &readings, bool wifiConnected,
  ...
  // Room temperature — display-only, not one of the 7 official
  // parameters (architecture.md 2.1 "Catatan suhu ruangan"); shown
  // visually distinct (grey, no threshold color logic) so it reads as
  // supporting info rather than a monitored/alerted value.
